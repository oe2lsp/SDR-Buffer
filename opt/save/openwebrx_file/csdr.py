"""
OpenWebRX csdr_old plugin: do the signal processing with csdr
...
class dsp:
    ...
    def __init__(self):
        ...
        self.name = "csdr"
        self.format_conversion = "csdr_old convert_u8_f"
    ...
    def chain(self,which):
        ...
        if self.csdr_dynamic_bufsize: any_chain_base+="csdr_old setbuf {start_bufsize} | "
        if self.csdr_through: any_chain_base+="csdr_old through | "
        any_chain_base+=self.format_conversion+(" | " if  self.format_conversion!="" else "") ##"csdr_old flowcontrol {flowcontrol} auto 1.5 10 | "
        if which == "fft":
            fft_chain_base = any_chain_base+"csdr_old fft_cc {fft_size} {fft_block_size} | " + \
                ("csdr_old logpower_cf -70 | " if self.fft_averages == 0 else "csdr_old logaveragepower_cf -70 {fft_size} {fft_averages} | ") + \
                "csdr_old fft_exchange_sides_ff {fft_size}"
            if self.fft_compression=="adpcm":
                return fft_chain_base+" | csdr_old compress_fft_adpcm_f_u8 {fft_size}"
            else:
                return fft_chain_base
        chain_begin=any_chain_base+"csdr_old shift_addition_cc --fifo {shift_pipe} | csdr_old fir_decimate_cc {decimation} {ddc_transition_bw} HAMMING | csdr_old bandpass_fir_fft_cc --fifo {bpf_pipe} {bpf_transition_bw} HAMMING | csdr_old squelch_and_smeter_cc --fifo {squelch_pipe} --outfifo {smeter_pipe} 5 1 | "
        if self.secondary_demodulator:
            chain_begin+="csdr_old tee {iqtee_pipe} | "
            chain_begin+="csdr_old tee {iqtee2_pipe} | " 
        chain_end = ""
        if self.audio_compression=="adpcm":
            chain_end = " | csdr_old encode_ima_adpcm_i16_u8"
        if which == "nfm": return chain_begin + "csdr_old fmdemod_quadri_cf | csdr_old limit_ff | csdr_old old_fractional_decimator_ff {last_decimation} | csdr_old deemphasis_nfm_ff 11025 | csdr_old fastagc_ff 1024 | csdr_old convert_f_s16"+chain_end
        elif which == "am": return chain_begin + "csdr_old amdemod_cf | csdr_old fastdcblock_ff | csdr_old old_fractional_decimator_ff {last_decimation} | csdr_old agc_ff | csdr_old limit_ff | csdr_old convert_f_s16"+chain_end
        elif which == "ssb": return chain_begin + "csdr_old realpart_cf | csdr_old old_fractional_decimator_ff {last_decimation} | csdr_old agc_ff | csdr_old limit_ff | csdr_old convert_f_s16"+chain_end

    def secondary_chain(self, which):
        secondary_chain_base="cat {input_pipe} | "
        if which == "fft":
            return secondary_chain_base+"csdr_old realpart_cf | csdr_old fft_fc {secondary_fft_input_size} {secondary_fft_block_size} | csdr_old logpower_cf -70 " + (" | csdr_old compress_fft_adpcm_f_u8 {secondary_fft_size}" if self.fft_compression=="adpcm" else "")
        elif which == "bpsk31":
            return secondary_chain_base + "csdr_old shift_addition_cc --fifo {secondary_shift_pipe} | " + \
                    "csdr_old bandpass_fir_fft_cc $(csdr_old '=-(31.25)/{if_samp_rate}') $(csdr_old '=(31.25)/{if_samp_rate}') $(csdr_old '=31.25/{if_samp_rate}') | " + \
                    "csdr_old simple_agc_cc 0.001 0.5 | " + \
                    "csdr_old timing_recovery_cc GARDNER {secondary_samples_per_bits} 0.5 2 --add_q | " + \
                    "CSDR_FIXED_BUFSIZE=1 csdr_old dbpsk_decoder_c_u8 | " + \
                    "CSDR_FIXED_BUFSIZE=1 csdr_old psk31_varicode_decoder_u8_u8"
    ...
    def start_secondary_demodulator(self):
        ...
        #open control pipes for csdr_old and send initialization data
        # print "==========> 1"
