...
ios = /iPad|iPod|iPhone|Chrome|Firefox/.test(navigator.userAgent);
...
function scale_setup()
{
	...
	//scale_canvas.addEventListener("touchstart", scale_canvas_mousedown, false);
	//scale_canvas.addEventListener("touchmove", scale_canvas_mousemove, false);
	//scale_canvas.addEventListener("touchend", scale_canvas_mouseup, false);
	//scale_canvas.addEventListener("touchcancel", scale_canvas_mouseup, false);
	resize_scale();
}
...
function scale_canvas_mousemove(evt) //in frequency-scale (top)
{
	var event_handled;
...
function canvas_mousemove(evt)
{
	...
	{
		//e("webrx-mouse-freq").innerHTML="touchmove "+evt.type;
...
var audio_buffer_maximal_length_sec=1.4; //actual number of samples are calculated from sample rate
var audio_buffer_decrease_to_on_overrun_sec=1.0;
var audio_flush_interval_ms=300; //the interval in which audio_flush() is called
...
function open_websocket()
{
	...
	//{
		...
		ws_url=protocol+(window.location.href.split("://")[1])+"ws/"; //guess automatically -> now default behaviour
	//}
...
function add_canvas()
{
	...
	new_canvas.addEventListener("touchmove", canvas_mousemove, false);
	//new_canvas.addEventListener("touchend", canvas_mouseup, false); //breaks chrom
	//new_canvas.addEventListener("touchcancel", canvas_mouseup, false);
	new_canvas.addEventListener("touchstart", canvas_mousedown, false);
...
function init_canvas_container()
{
	...
	//canvas_phantom.addEventListener("touchmove", canvas_mousemove, false);
	//canvas_phantom.addEventListener("touchend", canvas_mouseup, false);
	//canvas_phantom.addEventListener("touchstart", canvas_mousedown, false);
...
function openwebrx_init()
{
	...
	toggle_panel('openwebrx-panel-status',0);
...
function iosPlayButtonClick()
{
	...
	{
		audio_preinit();
		window.setTimeout(audio_init(),300);
		//audio_init();
...
function frequency_shift_absolute(delta)
{
//get offset, freq-span
//check if new qrg inside
//shift qrg
//if outside of visible reposition view


	lower_end= center_freq - bandwidth/2
	upper_end= center_freq + bandwidth/2
	newqrg= (center_freq + demodulators[0].offset_frequency + delta) // center qrg + relative qrg
	if((newqrg > lower_end) && (newqrg < upper_end))
	{
		
		demodulators[0].offset_frequency = demodulators[0].offset_frequency + delta;
		e("webrx-actual-freq").innerHTML=format_frequency("{x} MHz",center_freq+demodulators[0].offset_frequency,1e6,4);
		demodulators[0].set();		
		mkscale();		

		//check visible range
		visible=get_visible_freq_range()	
		//	Object { start: 21181097, center: 21258782, end: 21336468, bw: 155371, hps: 163.89345991561183 }
		if (newqrg > visible.end || newqrg < visible.start )
		{
			zoom_set(zoom_level)	
		}
	}
...
function modulation_rotate(direction)
{
	if (direction == -1 )
	{
		if (last_analog_demodulator_subtype == 'nfm') { demodulator_analog_replace('am') }
		else if (last_analog_demodulator_subtype == 'am') { demodulator_analog_replace('lsb') }
		else if (last_analog_demodulator_subtype == 'lsb') { demodulator_analog_replace('usb') }
		else if (last_analog_demodulator_subtype == 'usb') { demodulator_analog_replace('cw') }
		else if (last_analog_demodulator_subtype == 'cw') { demodulator_analog_replace('nfm') }
	}
	else if (direction == 1)
	{
		if (last_analog_demodulator_subtype == 'nfm') { demodulator_analog_replace('cw') }
		else if (last_analog_demodulator_subtype == 'am') { demodulator_analog_replace('nfm') }
		else if (last_analog_demodulator_subtype == 'lsb') { demodulator_analog_replace('am') }
		else if (last_analog_demodulator_subtype == 'usb') { demodulator_analog_replace('lsb') }
		else if (last_analog_demodulator_subtype == 'cw') { demodulator_analog_replace('usb') }
	}
}
...
document.addEventListener('keydown', function(e) {
	...
	document.activeElement.blur() //remove focus from input fields
	//console.log(keynum)
	if(keynum == 33) { //page up
		frequency_shift_absolute(6250)
	}
	if(keynum == 34) { //page up
		frequency_shift_absolute(-6250)
	}
	else if(keynum == 37) { //left
		frequency_shift_absolute(-1000)
	}
	else if(keynum == 39) { //right
		frequency_shift_absolute(1000)
	}
	else if(keynum == 38) { //up
		frequency_shift_absolute(100)
	}
	else if(keynum == 40) { //down
		frequency_shift_absolute(-100)
	}
	else if(keynum == 107 || keynum == 171) { //+
		zoomInOneStep();
	}
	else if(keynum == 109 || keynum == 173) { //-
		zoomOutOneStep();
	}
	else if(keynum == 122 || keynum == 70 || keynum == 16) { //f11, F, f
		fullscreen();
	}
	else if(keynum == 65) { //a
		//fm am lsb usb cw
