...
def handshake(myself):
    ...
    ws_key_toreturn=base64.b64encode(hashlib.sha1((ws_key+"258EAFA5-E914-47DA-95CA-C5AB0DC85B11").encode()).digest())
...
def send(myself, data, begin_id="", debug=0):
    ...
    while True:
        ...
        if from_end+len(begin_id)>base_frame_size:
            ...
            flush(myself)
            myself.wfile.write((header+data_to_send).encode())
            flush(myself)
